...
class QuestionGenerator:
    ...
    def generate_mcq(self,topic:str,difficulty:str='medium') -> MCQQuestion:
        """
        Generate a Multichoice question style question using the LLM.
        """
        try:
    ...
    def generate_fill_blank(self, topic: str, difficulty: str = 'medium') -> FillBlankQuestion:
        """
        Generate a fill-in-the-blank style question using the LLM.
        """
        try:
            parser = PydanticOutputParser(pydantic_object=FillBlankQuestion)
            question = self._retry_and_parse(fill_blank_prompt_template, parser, topic, difficulty)

            # Ensure the question contains a blank
            if "___" not in question.question:
                raise ValueError("Generated question does not contain a blank ('___').")
